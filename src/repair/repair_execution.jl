...
using .CoordinateKeys: create_edge_key_int, EdgeKeyInt
...
function load_repair_plan_from_json(json_file::String, mesh)
    ...
    for plan_dict in plan_data["insertion_sequence"]
        ...
        edge_key = create_edge_key_int(v1, v2)
...
function replace_both_interfaces!(
    ...
    # No verbose output here - handled by calling function
...
function execute_symmetric_repair!(
    ...
    if !plan.is_feasible
        if verbose
            println("      ✗ Symmetric repair plan not feasible:")
    ...
    success = replace_both_interfaces!(
        ...
        unified_mesh,
        plan.topology;
        verbose=false  # Suppress verbose output from sub-function
    )
